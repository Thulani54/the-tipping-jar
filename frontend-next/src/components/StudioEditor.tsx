...
// PNG / JPEG / clipboard export; gallery persisted via the Rust creators
...
const FONTS: Record<Font, { label: string; css: string; canvas: string }> = {
  display: { label: "Display", css: "var(--font-display)", canvas: "'Bricolage Grotesque'" },
  body: { label: "Body", css: "var(--font-body)", canvas: "'Hanken Grotesk'" },
...
export function StudioEditor({ token, slug }: { token: string | null; slug?: string | null }) {
  ...
  async function exportAs(fmt: "png" | "jpeg" | "clipboard", scale = 1) {
    setNote(null);
    ...
    a.href = fmt === "png" ? canvas.toDataURL("image/png") : canvas.toDataURL("image/jpeg", 0.92);
        ...
        <div className="space-y-4">
          ...
          <div className="card space-y-2 !p-4">
            <div className="grid grid-cols-2 gap-2">
              ...
              <button onClick={() => exportAs("clipboard")} className="btn-ghost !py-2.5 text-sm"><i className="bi bi-clipboard" /> Copy</button>
            </div>
